"""Base file to get inspiration from"""
...
def federated_sim(
    ...
    valloader = DataLoader(
        valset, batch_size=64, shuffle=True, num_workers=0  # validation set dataloader
    )
    testloader = DataLoader(
        testset, batch_size=64, shuffle=True, num_workers=0  # test set dataloader
    )
    ############################################################
    ...
    clientloaders = [
        DataLoader(
            Subset(trainset, client_indices[i]),
            batch_size=64,
            shuffle=True,
            num_workers=0,
        )
    ...
    while val_acc < 0.77 and current_round < max_rounds:
        ...
        client_ids = torch.randperm(num_clients)[
            :clients_per_round
        ].tolist()  # random selection of clients to participate
        local_weights = []
        temp_avg_loss = 0

        for client in client_ids:
            client_counter[client] += 1
            weights, loss = client_update(
                device,
                clientloaders[client],
                SmallCNN,
                current_weights,
                num_local_epochs,
                lr=lr,
