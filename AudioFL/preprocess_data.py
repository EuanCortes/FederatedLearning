...
def preprocess_data(src, dst, src_meta, n_processes=15):
    ...
    _ = pool.map(
        _preprocess_data,
        [
            (os.path.join(src, folder), os.path.join(dst, folder), src_meta)
...
def _preprocess_data(src_dst_meta):
    ...
    for filepath in sorted(glob.glob(os.path.join(src, "*.wav"))):
        ...
        data = librosa.core.resample(
            y=data.astype(np.float32), orig_sr=fs, target_sr=8000, res_type="scipy"
        )
        # zero padding
        if len(data) > 8000:
        ...
        with h5py.File(
            ...
            f["data"] = tmp_X
            f["label"] = np.array(
                [[int(dig), 0 if metaData[vp]["gender"] == "male" else 1]]
            )
        ...
        with h5py.File(
            os.path.join(dst, "AudioNet_{}_{}_{}.hdf5".format(dig, vp, rep)), "w"
        ) as f:
...
def create_splits(src, dst):
    ...
    splits = {
        ...
        "gender": {
            "train": [
                set([36, 47, 56, 26, 12, 57, 2, 44, 50, 25, 37, 45]),
    ...
    for split in range(5):
        for modus in ["train", "validate", "test"]:
            for task in ["digit", "gender"]:
                ...
                with open(
                    os.path.join(
                        dst, "AlexNet_{}_{}_{}.txt".format(task, split, modus)
                    ),
                    mode="w",
                ) as txt_file:
                ...
                with open(
                    os.path.join(
                        dst, "AudioNet_{}_{}_{}.txt".format(task, split, modus)
                    ),
                    mode="w",
                ) as txt_file:
                    for vp in splits[task][modus][split]:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Path to folder containing each participant's data directory.",
    )
    parser.add_argument(
        "--destination",
        "-dst",
        default=os.path.join(os.getcwd(), "preprocessed_data"),
        help="Destination where preprocessed data shall be stored.",
    )
    parser.add_argument(
        "--meta",
        "-m",
        default=os.path.join(os.getcwd(), "data", "audioMNIST_meta.txt"),
        help="Path to meta_information json file.",
    )
